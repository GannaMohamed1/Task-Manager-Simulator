...
class SystemMonitor:
    def __init__(self, master):
        ...
        self.disk_label = Label(master, text="Hard Drive Usage:")
        ...
        self.cpu_plot, = self.ax.plot([], [], color='red', label='CPU')
        self.memory_plot, = self.ax.plot([], [], color='yellow', label='Memory')
        self.disk_plot, = self.ax.plot([], [], color='green', label='Disk')
        self.ax.legend(loc='lower center')
        self.canvas = FigureCanvasTkAgg(self.fig, master=master)
        self.canvas.get_tk_widget().grid(row=0, column=2, rowspan=4, padx=10, pady=10)
        self.animation = FuncAnimation(self.fig, self.update_graph, frames=120, interval=1100)
    ...
    def send_mail(self):
        ...
        sender_email = "gannamohamed318@hotmail.com"
        receiver_email = "ganna.elsayed@ejust.edu.eg"
        password = "GannaMohamed2003"
        ...
        message.attach(MIMEText("PC Data File Attached", "plain"))
